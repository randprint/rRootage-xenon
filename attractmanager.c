...
#define PREF_FILE "uda://rrootage/rr.prf"
...
void moveTitleMenu() {
	//printf("moveTitleMenu1\n");
  int pad = getPadState();
	//printf("moveTitleMenu2\n");
  int btn = getButtonState();
	//printf("moveTitleMenu3\n");
  int bs = slcStg;
	//printf("moveTitleMenu4\n");
  if ( pad & PAD_DOWN ) {
    if ( mnp ) {
  ...
  if ( slcStg != bs ) {
    mnp = 0;
	//printf("moveTitleMenu5\n");
    initTitleStage(slcStg);
	//printf("moveTitleMenu6\n");
    titleCnt = 0;
  }
  if ( mnp && (btn & PAD_BUTTON1) ) {
    if ( slcStg == QUIT_STAGE_NUM  ) {
	//printf("moveTitleMenu7\n");
      quitLast();
    } else if ( slcStg < 0 ) {
      mnp = 0;
	//printf("moveTitleMenu8\n");
      setMode(MODE_NUM+slcStg);
	//printf("moveTitleMenu9\n");
    } else {
      hiScore.stage = slcStg;
	//printf("moveTitleMenu10\n");
      initGame(slcStg);
	//printf("moveTitleMenu11\n");
    }
  }
  if ( mnp && (btn & PAD_BUTTON2) ) {
    mnp = 0;
	//printf("moveTitleMenu12\n");
    setMode((mode+1)%MODE_NUM);
	//printf("moveTitleMenu13\n");
    initTitleStage(slcStg);
	//printf("moveTitleMenu14\n");
    titleCnt = 0;
  }
  titleCnt++;
}

//senquack - 2nd note: disabling other stuff in this didn't help line drawing bug
//senquack - note - disabling everything here after drawTitleBoard causes unusual stuff to happen:
void drawTitle() {
  int i;
  int r, g, b;
  int sx, sy;
